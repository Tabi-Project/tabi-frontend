...
import { X, SubCta, SectionHead } from "../ui";
...
export function ConfirmationStep({
  ...
  const t = useTranslations("Bootcamp.modal.confirmation");

  const fullName =
    [firstName, lastName].filter(Boolean).join(" ").trim() || "there";

  // Pre‑filled WhatsApp message – uses a translation key with {fullName} and {refId}
  const waMsg = encodeURIComponent(t("whatsappMessage", { fullName, refId }));
  const waUrl = `https://wa.me/${WA_NUMBER}?text=${waMsg}`;

  // Email subject & body – also translatable
  const mailSub = encodeURIComponent(t("emailSubject", { refId }));
  const mailBdy = encodeURIComponent(t("emailBody", { fullName, refId }));
    ...
    <div>
      ...
      >
        ...
        <div className="space-y-3">
          {[
            {
              icon: "👤",
              text: t("items.fullName", { fullName })
            },
            {
              icon: "🔖",
              text: t("items.referenceId", { refId })
            },
            {
              icon: "📸",
              text: t("items.screenshot")
            }
          ].map((it, i) => (
      ...
      >
        <motion.div
          ...
          className="w-full py-3.5 rounded-xl font-semibold text-sm text-white flex items-center justify-center gap-2.5 cursor-pointer"
          style={{
            background: "#25D366",
